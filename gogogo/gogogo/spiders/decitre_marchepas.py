...
for row in range(1, xlsx_workbook.max_row):
    urls.append('http://www.decitre.fr/livres/'+str(xlsx_workbook.cell(row = row, column = 2).value)+'.html')
    eanCode.append((xlsx_workbook.cell(row = row, column = 1).value,xlsx_workbook.cell(row = row, column = 2).value))
def nettoyer(elm):
    return (elm.replace('\n','').strip())
